...
@endif
    ...
    <form action="{{ route('historiales.destroy', $historiale->id) }}" method="POST">
        ...
        {{-- Todos los que tienen permiso para 'ver' (Alumno, Prof, Admin) --}}
        @can('view', $historiale)
        ...
        {{-- Solo los que tienen permiso para 'actualizar' (Prof de este grupo y Admin) --}}
        @can('update', $historiale)
            <a class="btn btn-sm btn-icon btn-success" href="{{ route('historiales.edit', $historiale->id) }}" data-bs-toggle="tooltip" title="Editar Calificación">
                <i class="fas fa-edit"></i>
            </a>
        @endcan

        {{-- Solo los que tienen permiso para 'borrar' (Admin) --}}
        @can('delete', $historiale)
            @csrf
            @method('DELETE')
