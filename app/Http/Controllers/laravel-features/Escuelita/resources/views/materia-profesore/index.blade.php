    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="card shadow border-0 mb-4">
                <div class="card-body">
                    <form action="{{ route('materia-profesores.index') }}" method="GET" class="row g-3 align-items-center">
                        <div class="col-md">
                            <div class="input-group">
                                <span class="input-group-text"><i class="fas fa-search"></i></span>
                                <input type="text" name="search" class="form-control" placeholder="Buscar por Profesor o Materia..." value="{{ request('search') }}">
                            </div>
                        </div>
                        <div class="col-md-auto">
                            <button type="submit" class="btn btn-info">Buscar</button>
                            @if(request('search'))
                                <a href="{{ route('materia-profesores.index') }}" class="btn btn-secondary">Limpiar</a>
                            @endif
                        </div>
                    </form>
                </div>
                        ...
                        <table class="table table-hover align-middle">
                            ...
                            <tbody>
                                @forelse ($materiaProfesores as $materiaProfesore)
                                    <tr>
                                        <td>{{ ++$i }}</td>
                                        <td>{{ $materiaProfesore->profesor->nombre ?? 'N/A' }} {{ $materiaProfesore->profesor->apellido_paterno ?? '' }}</td>
