    ...
    public function index(Request $request): View
{
    ...
    if ($user->role === 'profesor') {
        // Un profesor SOLO ve historiales de SUS grupos
        $query->whereHas('grupo', function ($q) use ($user) {
            $q->where('profesor_id', $user->matricula);
        });

    } else if ($user->role === 'alumno') {
        // Un alumno SOLO ve SUS propios historiales
        $query->where('alumno_matricula', $user->matricula);
    }
    // Los Admins ven todo (no se aplica filtro)
    // =================================

    $historiales = $query->when($search, function ($query, $search) {
    ...
    {
        // Autorizamos primero (esto usará tu Política)
        $this->authorize('update', $historiale);
        ...
        if ($user->role === 'admin') {
            // El Admin puede actualizar todos los datos validados
            $historiale->update($datosValidados);
        
        } elseif ($user->role === 'profesor') {
            // El Profesor SÓLO puede actualizar la calificación
            $historiale->fill([
                'calificacion' => $datosValidados['calificacion'] ?? null
            ]);
            $historiale->save();
        }
        // Los alumnos no pueden actualizar (ya fueron bloqueados por la Policy)
        // =================================================
