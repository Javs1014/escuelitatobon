    ...
    public function index(Request $request): View
{
    ...
    $query = Grupo::with('materia', 'profesor', 'horarios');

    // ======== LÓGICA DE ROLES ========
    if ($user->role === 'profesor') {
        // Un profesor SOLO ve sus propios grupos
        $query->where('profesor_id', $user->matricula);

    } else if ($user->role === 'alumno') {
        // Un alumno NO VE NINGÚN grupo en esta lista
        $query->whereRaw('1 = 0'); // Fuerza a no devolver resultados
    }
    // Los Admins ven todo (no se aplica filtro)
    // =================================

    $grupos = $query->when($search, function ($query, $search) {
    ...
    public function create(): View
    {
        ...
        // Obtenemos profesores con 'matricula' como llave
        ...
        // Mostramos la vista 'grupo.create'
    ...
    {
        // Cargamos los horarios existentes del grupo para el formulario
        $grupo->load('horarios');
