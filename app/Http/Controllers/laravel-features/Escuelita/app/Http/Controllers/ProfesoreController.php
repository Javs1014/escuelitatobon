...
class ProfesoreController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $query->whereRaw('1 = 0'); // No mostrar nada si no es admin
    }
        ...
        $profesores = Profesore::with('area')
            ->when($search, function ($query, $search) {
                return $query->where(function ($q) use ($search) {
                    // CAMBIO: Añadir búsqueda por matrícula
        ...
        return view('profesore.index', compact('profesores'))
            // CAMBIO: Paginación. No usar ++$i, usar el paginador.
    ...
    public function destroy(Profesore $profesore): RedirectResponse
{
    try {
        ...
        // 2. Si lo logra, manda mensaje de éxito
        return Redirect::route('profesores.index')
        ...
        // Si falla (código 23000), es por un "hijo" (un grupo, etc.)
        if ($e->getCode() == '23000') {
            // 4. Manda un mensaje de ERROR amigable, en lugar de la página roja
            return Redirect::route('profesores.index')
                ->with('error', 'No se puede eliminar el profesor, ya que está asignado a uno o más grupos.');
        }

        // Si es otro error, lo muestra
        return Redirect::route('profesores.index')
